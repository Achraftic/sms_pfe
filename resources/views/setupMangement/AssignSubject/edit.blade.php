...
@section('content')
    <main class=" md:p-10 p-6 min-h-screen">
        ...
        <form action="{{ route('assign.subject.update',$data[0]->class_id) }} " method="post"
...
<div class=" gap-7 sm:grid grid-cols-2    sm:space-y-0 space-y-6">
                    ...
                    <div class="col-span-2 mb-4 ">
                        ...
                        <select name="class_id" class="form-select w-full" id="class_id">
                            ...
                            @foreach ($stdent_class as $key => $value)
                                <option value={{ $value->id }}  {{($i->class_id==$value->id)?"selected":""}} > {{ $value->name }} </option>
                            @endforeach
                    ...
                    <div>
                        ...
                        <select name="subject_id[] " class="form-select w-[90%]" id="subject_id">
                            ...
                            @foreach ($subject as $key => $value)
                                <option value={{ $value->id }}  {{($i->subject_id==$value->id)?"selected":""}} > {{ $value->name }} </option>
                            @endforeach
                        </select>
                        {{--  @error('name')
                    ...
                    <div class="grid grid-cols-3  gap-3">
                        <div>
                            ...
                            <input id="amount" class="form-input w-20" name="full_mark[] " value="{{$i->full_mark}}" type="number" />
                            @error('amount')
                                <span class="  text-red-400  text-sm p-1"> {{ $message }} </span>
                            @enderror
                        ...
                        <div>
                            ...
                            <input id="amount" class="form-input w-20" name="pass_mark[]" value="{{$i->pass_mark}}" type="number" />
                            @error('pass_mark')
                                <span class="  text-red-400  text-sm p-1"> {{ $message }} </span>
                            @enderror
                        ...
                        <div>
                            ...
                            <input id="amount" class="form-input w-20" name="subjective_mark[]" value="{{$i->subjective_mark}}" type="number" />
